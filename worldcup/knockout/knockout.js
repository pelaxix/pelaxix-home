(() => {
  ...
  const ROUND_LAYOUT = [
    { title: "Round of 32", subtitle: "16 matches", stage: "r32", ids: [75, 78, 73, 76, 84, 83, 82, 81, 74, 77, 79, 80, 87, 86, 85, 88], step: 1, offset: 0 },
    { title: "Round of 16", subtitle: "8 matches", stage: "r16", ids: [90, 89, 93, 94, 91, 92, 95, 96], step: 2, offset: 0.5 },
    { title: "Quarterfinals", subtitle: "4 matches", stage: "qf", ids: [97, 98, 99, 100], step: 4, offset: 1.5 },
    { title: "Semifinals", subtitle: "2 matches", stage: "sf", ids: [101, 102], step: 8, offset: 3.5 },
    { title: "Final", subtitle: "1 match", stage: "final", ids: [104], step: 16, offset: 7.5, final: true }
  ];
  ...
  function makeBracketScrollable() {
    ...
    if (bracketHintEl) {
      bracketHintEl.textContent = "Swipe sideways between rounds · scroll up and down through the bracket";
    }
  ...
  function bracketGeometry() {
    ...
    const pitch = mobile ? 92 : 98;
    const cardHeight = mobile ? 66 : 70;
  ...
  function matchCard(match, context, round, index, geometry) {
    ...
    const branchClass = !round.final ? (index % 2 === 0 ? "branch-top" : "branch-bottom") : "";
    const classes = `${round.final ? "final-card " : ""}${finished ? "is-finished " : ""}${branchClass}`.trim();
  ...
  function updateColumnFocus() {
    ...
    scroller.querySelectorAll(".bracket-round").forEach((round) => {
      const center = round.offsetLeft + (round.offsetWidth / 2);
      const distance = Math.abs(center - viewportCenter);
      const focus = Math.max(0, Math.min(1, 1 - (distance / Math.max(scroller.clientWidth * 0.75, 1))));
      round.style.opacity = String(0.55 + (focus * 0.45));
      round.style.transform = `scale(${0.97 + (focus * 0.03)})`;
      round.classList.toggle("is-active", focus >= 0.7);
    });
  }
