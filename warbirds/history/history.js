const workerHost=["hamilton-warbird-watch","pelaxix","workers","dev"].join(".");
...
function getPlaneUrl(hex){
  return hex?`https://globe.airplanes.live/?icao=${encodeURIComponent(hex)}`:null;
}
...
function activityItems(scan){
  return Array.isArray(scan.aircraft)?scan.aircraft:Array.isArray(scan.liveMatches)?scan.liveMatches:[];
}

function scanKind(scan){
  if(scan.error) return "error";
  const aircraft=activityItems(scan);
  if(aircraft.some(item=>item.airborne)) return "airborne";
  if(aircraft.length) return "detected";
  return "quiet";
}
...
function scanLabel(scan){
  ...
  if(kind==="error") return "Scan error";
  if(!aircraft.length) return "No watched aircraft detected";
  if(aircraft.length===1) return aircraft[0].label||aircraft[0].aircraft||aircraft[0].registration||"Aircraft detected";
...
function scanDetail(scan){
  ...
  if(!aircraft.length) return scan.source==="manual"?"Manual check":"Scheduled check";
  return aircraft.map(item=>{
    ...
    return route?`<a class="plane-link" href="${route}" target="_blank" rel="noopener noreferrer">${name}${registration}</a>`:`${name}${registration}`;
  }).join(" · ");
}

function render(){
  const scans=showActivityOnly?allScans.filter(scan=>scanKind(scan)!=="quiet"):allScans;
  if(!scans.length){
    showEmpty(showActivityOnly?"No activity in this window":"No scans recorded yet",showActivityOnly?"Try showing all scans, or check back after an aircraft is detected.":"The first completed scheduled scan will appear here.");
    return;
  }
  ...
  scans.forEach(scan=>{
    ...
    badge.textContent=kind==="airborne"?"Airborne":kind==="detected"?"Detected":kind==="error"?"Error":"Quiet";
...
async function loadHistory(manual=false){
  ...
  try{
    ...
    allScans=Array.isArray(payload.scans)?payload.scans:Array.isArray(payload.history)?payload.history:[];
    allScans.sort((first,second)=>Date.parse(second.at||second.timestamp)-Date.parse(first.at||first.timestamp));
    render();
    historyStatus.textContent=allScans.length?`${allScans.length} scan${allScans.length===1?"":"s"} recorded · last 48 hours`:"No scans recorded yet";
  }catch(error){
    console.error("Could not load scan history",error);
    showEmpty("Scan history is not online yet","The history page is ready. The Worker needs its final scan-log update before entries can appear here.");
    historyStatus.textContent="Waiting for the scan-log update";
  }finally{
